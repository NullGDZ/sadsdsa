    ...
    updateBotStatus() {
     ...
     update() {
        ...
        if (this.player.decoded.botsMax === undefined) {
            ...
            this.divs.rows.bots.innerHTML = "Bots: " + this.player.decoded.currentBots + "/" + (this.player.decoded.botsMax || 0);
        }
        const percentage = this.player.decoded.currentBots / (this.player.decoded.botsMax || 1) * 100;
        const botIndicator = document.getElementById("botIndicator");
        if (botIndicator) {
            botIndicator.style.width = percentage + "%";
        }
        if (!window.loadedBar) {
            window.loadedBar = !0;
            let BotsDetected = setInterval(() => {
                if (this.player.decoded.botsMax !== 0 && !window.loadedBarBots) {
                    ...
                    if (c) {
              noUiSlider.create(c, {
                start: parseInt(this.player.decoded.botsMax),
                step: 1,
                range: {
                  min: 1,
              ...
              // Fix slider change event
              c.noUiSlider.on('change', function(values, handle) {
                const currentTime = Date.now();
                if (sliderLocked) {
                  c.noUiSlider.set(window.sliderValue);
                  return
                }
                ...
                window.sliderValue = parseInt(values[handle].toString().replace(/\s+/g, ''));
                window.botsDyn = window.sliderValue;
                
                if (window.botr) {
                  thot.socket.send({
                    req: 99,
                    botdynv2: window.botsDyn
                  })
                }
                
                // Update bot indicator immediately after slider changes
                const percentage = thot.player.decoded.currentBots / (window.sliderValue || 1) * 100;
                const botIndicator = document.getElementById("botIndicator");
                if (botIndicator) {
                  botIndicator.style.width = percentage + "%";
                }
              ...
              c.noUiSlider.on('slide', function(values, handle) {
                if (!sliderLocked) {
                  let displayValue = parseInt(values[handle]);
                  if (c.querySelector('.noUi-tooltip')) {
                    c.querySelector('.noUi-tooltip').textContent = displayValue;
                  }
                }
              });
            }
